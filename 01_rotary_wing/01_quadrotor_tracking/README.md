...
This simulation investigates a fundamental robustness question in
nonlinear UAV control:

> **How does a nonlinear quadrotor trajectory-tracking controller behave
> when the mathematical model used by the controller does not exactly
> match the actual plant, and when an external disturbance is
> introduced?**

The study uses a 6-DOF rigid-body quadrotor model and a nonlinear
...
The same controller is evaluated under three conditions:
  ...
  ----------------------- ----------------------- -----------------------
  **A --- Nominal**       Controller model =      None
                          actual plant            

  **B --- Model           20% mass increase +     None
  uncertainty**           modified inertia        

  **C --- Uncertainty +   Same uncertainty as B   Smooth lateral force
  disturbance**                                   
  -----------------------------------------------------------------------

The purpose is not simply to generate a successful animation. The
...
The first Yantra Gyan quadrotor study uses a **nonlinear geometric
controller** consisting of:

1.  a nonlinear translational/position controller, and

2.  a geometric attitude controller formulated using rotation matrices
    on (SO(3)).
...
The translational controller generates the force required to track the
desired position. The desired force direction determines how the
quadrotor must orient its thrust axis. The attitude controller then
generates the body moments required to achieve that orientation.

------------------------------------------------------------------------

## 3. Why was geometric nonlinear control selected for Yantra Gyan #01?

The controller is **not selected because it is universally the best
controller for quadrotors**.

It is selected because it provides a strong and physically meaningful
starting point for this series.

### 3.1 It is genuinely nonlinear

The controller operates with the nonlinear rigid-body dynamics rather
than first linearizing the vehicle around a hover equilibrium.

The rotational dynamics retain the coupling term

\$\$

`\boldsymbol{\omega}`{=tex}`\times `{=tex}J`\boldsymbol{\omega}`{=tex}.

\$\$

This makes it appropriate for a series focused on nonlinear modelling
and control.

### 3.2 It matches the physics of a quadrotor

A quadrotor fundamentally controls its translational motion by changing
the direction of its total thrust vector.
...
This provides a clean connection between the mathematics and the
physical vehicle.

### 3.3 It provides a clean attitude representation

The attitude tracking error is calculated using rotation matrices rather
than directly treating Euler-angle errors as three independent scalar
errors.
...
This is useful because attitude belongs naturally to the rotation
manifold (SO(3)).

### 3.4 It provides a strong reference for future controllers

The same vehicle, trajectory and evaluation metrics can later be used to
compare:
...
The mathematical formulation is rigorous enough for a control-system
study, while the implementation remains compact enough that another
student or engineer can follow the code.

The objective is:

> **Understand the controller before trying to make it more
> complicated.**
...
`\mathbf `{=tex}e_p=`\mathbf `{=tex}p-`\mathbf `{=tex}p_d
...
`\mathbf `{=tex}e_v=`\mathbf `{=tex}v-`\mathbf `{=tex}v_d.
...
m_c(`\mathbf `{=tex}a_c-`\mathbf `{=tex}g),
...
`\frac{\mathbf F_c}{\|\mathbf F_c\|}`{=tex}.

\$\$

The desired thrust direction and desired yaw are then used to construct
(R_d).
...
R\^TR_d`\boldsymbol`{=tex}`\omega`{=tex}\_d.
...
`\boldsymbol`{=tex}`\omega`{=tex}`\times `{=tex}J_c`\boldsymbol`{=tex}`\omega`{=tex}
...
\[T,`\tau`{=tex}\_x,`\tau`{=tex}\_y,`\tau`{=tex}\_z\]\^T

\$\$

is converted into four rotor commands through the control-allocation
matrix.
...
Yantra Gyan is intentionally focused on **nonlinear control and
progressively more advanced control/learning methods**.

PID and LQR are extremely useful engineering tools, but they are not the
focus of this series.

They may be discussed later as comparison/reference controllers, but the
primary experiments will investigate nonlinear and learning-based
approaches.

The objective is not to claim that nonlinear control is always superior.
...
> **Do not choose gains simply because they make one simulation look
> good.**

The gain-selection process should have a physical and analytical basis.

The initial gains for the translational controller are interpreted
through the approximate tracking-error dynamics.

Ignoring disturbances and model mismatch,

\$\$

`\ddot{\mathbf e}`{=tex}\_p

\+

K_v`\dot{\mathbf e}`{=tex}\_p
...
s\^2+2`\zeta`{=tex}`\omega`{=tex}\_n s+`\omega`{=tex}\_n\^2=0.
...
k_v=2`\zeta`{=tex}`\omega`{=tex}\_n.

\$\$

This provides a systematic starting point based on desired natural
frequency and damping ratio.

The attitude gains are then selected using the same engineering
philosophy: start from the expected rotational response and increase the
gains only as necessary to achieve adequate attitude tracking without
...
A controller that has a very small steady-state error but an
unacceptable transient response is not considered a good design.

------------------------------------------------------------------------

## 7.3 Control effort

Increasing gains indefinitely can reduce tracking error while
dramatically increasing control demand.
...
The controller should achieve the required performance without demanding
unreasonable thrust or moments.
...
0`\leq`{=tex}`\omega`{=tex}*i`\leq`{=tex}`\omega`{=tex}*{`\max`{=tex}}.

\$\$

A controller that produces excellent tracking while continuously
saturating the motors is **not considered a successful controller
design**.
...
This is particularly important for Yantra Gyan #01 because the main
research question concerns **model mismatch**.
...
The controller must subsequently be tested against model uncertainty and
disturbance.
...
A controller that performs well only under the exact model used during
design should not be described as robust.
...
Case B is retained and a smooth lateral disturbance is applied from
15--25 s.
...
`\text{physical control authority}`{=tex}.
...
The source code is intentionally written for readability rather than
minimum line count.
...
4.  and, where useful, **which mathematical relationship it
    implements.**
...
The comments should explain the engineering logic, not merely translate
Python syntax into English.

------------------------------------------------------------------------

## 16. Modelling assumptions and limitations

This is a controlled simulation benchmark, not a high-fidelity
prediction of a particular commercial UAV.
...
The plant state is represented using Euler angles. The attitude
controller uses geometric rotation-matrix errors, but the Euler-angle
state representation still has a singularity near:

\$\$

`\theta`{=tex}=`\pm90`{=tex}\^`\circ`{=tex}.

\$\$

Future aggressive-flight and VTOL transition studies should move to
quaternion or direct (SO(3)) state propagation.
...
The first experiment establishes the methodology that will be used
...
The intention is not to add complexity simply for the sake of using a
more advanced algorithm.

Each new method should be introduced because the previous experiment
exposes a specific limitation or research question.
...
The objective is to understand where each method is useful, what
assumptions it makes, what limitations it introduces, and whether the
...
Instead of representing uncertainty only as a fixed parameter mismatch,
the next experiment will introduce a physically meaningful event:

> **The quadrotor experiences a sudden change in payload/mass during
> flight.**
